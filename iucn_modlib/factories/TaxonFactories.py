...
def TaxonFactoryRedListBatch(species, source, fixElevation = True, fixHabitats = True):
    ...
    tax = Taxon(
        ...
        kingdom            = unwrap(taxonomy,'kingdomName'),#taxonomy['kingdomName'].values[0],
        phylum             = unwrap(taxonomy, 'phylumName'),#taxonomy['phylumName'].values[0],
        class_             = unwrap(taxonomy,'className'),#taxonomy['className'].values[0],
        order              = unwrap(taxonomy, 'orderName'),#taxonomy['orderName'].values[0],
        family             = unwrap(taxonomy,'familyName'),#taxonomy['familyName'].values[0],
        genus              = unwrap(taxonomy,'genusName'),#taxonomy['genusName'].values[0],
        ...
        marine_system      = unwrap(assessments,'systems'),#assessments['systems'].values[0].find('Marine') != -1,
        ...
        eoo_km2            = unwrap(all_other_fields,'EOO.range'),#all_other_fields['EOO.range'].values[0],
